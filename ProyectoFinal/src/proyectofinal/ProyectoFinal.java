...
public class ProyectoFinal {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<String> baseDeDatosText=baseMenu.obtenerBD();
        ArrayList<Platillo> men =new ArrayList<Platillo>();
        Platillo plaAux =new Platillo(30, "holaa", 0, "Eliminar", 0);
        StringTokenizer token;
        /*for (int i = 0; i < baseMenu.obtenerBD().size(); i++) {
            token=new StringTokenizer(baseMenu.obtenerBD().get(i),",");
            plaAux.setId(Integer.parseInt(token.nextToken()));
            plaAux.setNombre(token.nextToken());
            plaAux.setPrecio(Double.parseDouble(token.nextToken()));
            plaAux.setDescripcion(token.nextToken());
            plaAux.setExistencias(Integer.parseInt(token.nextToken()));
            System.out.println(plaAux);
            men.add(plaAux);
        }
        System.out.println(baseMenu.obtenerBD().size());
        elMenu.setMenu(men);
        //
...
try{ 
           ...
           // JOptionPane.showMessageDialog(null, "Bien, tu nombre es "+nombre);
            ...
            switch(opcion){
                ...
                case 5://IMPRIMIR BASE DE DATOS PLATILLOS
                    JOptionPane.showMessageDialog(null,elMenu.getMenu());
                    break;

                case 6://IMPRIMIR BASE DE DATOS VENTAS
                    JOptionPane.showMessageDialog(null,pedido.getVentasConcretadas());
                    break;
                default:
                    break;
            }
    ...
    public static void moduloPedidos(int banderaMenu, Menu menu,Pedido ventas){
        ...
        for (int i = 0; i < opcionCuantosPlatillos; i++) {
            ...
            //ESCOGER PLATILLO
            do {
                opcionID=Integer.parseInt(JOptionPane.showInputDialog("Ingrese el ID del platillo "+(i+1)+" que desea ordenar:"));
